...
if [ ! -z "$cas_version" ]
  then
	...
	docker build --tag="huuhuy/cas:v$cas_version" --build-arg cas_version=$cas_version . && echo "Built CAS image successfully tagged as v$cas_version" && docker images "huuhuy/cas:v$cas_version"
  else
